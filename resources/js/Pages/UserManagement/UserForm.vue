            ...
            <div class="max-w-3xl mx-auto space-y-6">
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/60 shadow-sm p-6">
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <BaseInput v-model="form.employee_id" label="Employee Code (optional)" placeholder="EMP-001" :error="form.errors.employee_id" />
                        ...
                        <div class="rounded-xl border border-emerald-100 bg-emerald-50/40 p-4 space-y-2">
                            <label class="block text-sm font-bold text-emerald-800">
                                🔗 Link to Employee Profile
                            </label>
                            <p class="text-xs text-emerald-700">
                                Mapping a user to an employee record enables HR access (attendance, payroll, leaves, etc.).
                                Only employees not yet linked to any user account are listed.
                            </p>
                            <select
                                v-model="form.employee_link_id"
                                class="w-full rounded-xl border-emerald-200 bg-white focus:border-emerald-500 focus:ring-emerald-500/20 text-sm font-medium text-gray-800 mt-1"
                            >
                                <option :value="null">— No employee link (standalone user) —</option>
                                <option v-for="emp in unlinkedEmployees" :key="emp.id" :value="emp.id">
                                    {{ emp.name }}
                                </option>
                            </select>
                            <p v-if="form.errors.employee_link_id" class="text-red-500 text-xs mt-1">{{ form.errors.employee_link_id }}</p>
                            <p v-if="isEditing && linkedEmployeeId && form.employee_link_id !== linkedEmployeeId" class="text-amber-600 text-xs font-semibold mt-1">
                                ⚠️ Saving will detach the previously linked employee and link the new selection.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <label
                                    v-for="role in roles"
                                    :key="role.id"
                                    class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 bg-white/50 hover:border-emerald-500/30 cursor-pointer transition-all"
                                    :class="{ 'ring-2 ring-emerald-500/20 border-emerald-500': form.role_ids.includes(role.id) }"
                                >
...
const props = defineProps({
    ...
    departments:        Array,
    locations:          Array,
    unlinkedEmployees:  { type: Array,  default: () => [] },
    linkedEmployeeId:   { type: Number, default: null },
});
...
const form = useForm({
    ...
    employee_link_id:   props.linkedEmployeeId ?? null,
    password:           '',
