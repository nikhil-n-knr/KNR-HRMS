    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
      ...
      <section class="rounded-3xl border border-indigo-200 bg-gradient-to-br from-indigo-50 via-blue-50 to-white p-5 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <a v-if="opsLaunchers.can_view_devops_global && opsLaunchers.devops_global_url" :href="opsLaunchers.devops_global_url" class="rounded-2xl border border-slate-200 bg-white px-4 py-3 hover:border-emerald-300 transition-colors">
          ...
          <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">
            ...
            <div class="mt-2 space-y-1 max-h-20 overflow-auto pr-1">
              <a v-for="project in opsLaunchers.project_devops || []" :key="`devops-link-${project.id}`" :href="project.url" class="block text-xs font-bold text-cyan-700 hover:text-cyan-800 truncate">
                {{ project.name }}
              </a>
              <p v-if="!(opsLaunchers.project_devops || []).length" class="text-xs font-semibold text-slate-400">No linked project dashboards.</p>
            </div>
          </div>
        </div>
      ...
      <section class="grid grid-cols-1 xl:grid-cols-[1.4fr_1fr] gap-4">
        ...
        <article class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm space-y-3">
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
              <p class="text-[10px] font-black uppercase tracking-wider text-slate-500">CL Plan</p>
              <p class="text-xl font-black text-slate-900">{{ rangeSummary.checklist_planned_hours || 0 }}h</p>
            </div>
            <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
              <p class="text-[10px] font-black uppercase tracking-wider text-slate-500">CL Actual</p>
      ...
      <section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
        <article v-for="card in planCards" :key="card.key" class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div class="mt-3 text-[11px] font-semibold text-slate-500 space-y-1">
            ...
            <p v-if="card.meta?.checklist_plan_hours !== undefined">CL Plan {{ card.meta.checklist_plan_hours }}h | CL Actual {{ card.meta.checklist_actual_hours }}h</p>
      ...
      <section class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between gap-3">
          ...
          <p class="text-xs font-semibold text-slate-500">Estimate vs checklist plan vs actual vs timesheet</p>
        </div>
          ...
          <table class="w-full min-w-[860px]">
            <thead class="bg-slate-50">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">CL Plan</th>
                <th class="px-4 py-3 text-left text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">CL Actual</th>
              ...
              <tr v-for="task in tasks.data" :key="task.id" class="border-b last:border-b-0" :class="taskRowClass(task)">
                ...
                <td class="px-4 py-3">
                  <div class="space-y-2">
                    ...
                    <p class="text-[11px] font-semibold text-slate-500">
                      Est {{ task.reporting?.task_estimated_hours ?? 0 }}h | CL Plan {{ task.reporting?.checklist_estimated_hours ?? 0 }}h | CL Actual {{ task.reporting?.checklist_actual_hours ?? 0 }}h | TS {{ task.reporting?.timesheet_hours_me ?? task.reporting?.timesheet_hours_total ?? 0 }}h
                    </p>
  ...
  <div v-if="checklistModal.open" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="w-full max-w-5xl max-h-[92vh] overflow-hidden bg-white rounded-3xl border border-slate-200 shadow-2xl flex flex-col">
      <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
        <div>
          <h3 class="text-xl font-black text-slate-900">Checklist Workspace</h3>
      ...
      <div class="px-6 py-4 border-b border-slate-200 grid grid-cols-1 lg:grid-cols-3 gap-3">
        <input
          ...
          placeholder="Search checklist content"
        >
      ...
      <div v-if="isChecklistEditingLocked" class="px-6 py-3 border-b border-rose-200 bg-rose-50">
        ...
        <p class="text-sm font-semibold text-rose-700 mt-1">Checklist marking and effort updates are disabled because this task is locked.</p>
      </div>
      ...
      <div class="px-6 py-4 border-b border-slate-200 grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="rounded-2xl border border-slate-200 p-3 space-y-2">
          ...
          <div class="grid grid-cols-1 md:grid-cols-[1fr_120px_auto] gap-2">
            <input v-model="checklistModal.newItem.content" type="text" class="h-9 rounded-lg border border-slate-200 px-2 text-sm font-semibold" placeholder="Checklist item content">
          ...
          <div v-for="item in checklistModal.items" :key="item.id" class="rounded-xl border border-slate-200 px-3 py-3">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
              <div>
                ...
                <p class="text-[11px] text-slate-500 font-semibold">Plan {{ minutesToHours(item.planned_minutes) }}h | Actual {{ minutesToHours(item.actual_minutes) }}h | Work Date {{ item.work_date || '-' }}</p>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-4 gap-2 lg:w-[640px]">
                <input
              ...
              <p v-if="isChecklistDateOverwritten(item)" class="text-[11px] font-bold text-rose-600">
                Work date was already saved as {{ item.work_date }} and is now being overwritten.
              </p>
            </div>
          </div>
...
const deleteSelectedPreset = async () => {
  ...
  try {
    ...
    const response = await fetch(`${route('employee.work.defaults.clear')}?preset_key=${encodeURIComponent(presetState.selectedKey)}`, {
      method: 'DELETE',
      headers: {
    ...
    if (!response.ok) throw new Error(data.message || 'Unable to delete preset.');

    setFlash(data.message || 'Preset deleted.');
...
const clearDefaultFilters = async () => {
  try {
    ...
    const response = await fetch(route('employee.work.defaults.clear'), {
      method: 'DELETE',
      headers: {
        Accept: 'application/json',
        'X-CSRF-TOKEN': csrfToken,
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
      throw new Error(data.message || 'Unable to clear defaults.');
    }

    setFlash(data.message || 'Defaults cleared.');
    refreshWorkbench();
  } catch (error) {
    setError(error.message);
  } finally {
...
const isChecklistDateOverwritten = (item) => {
  const savedDate = item?.work_date || '';
...
const loadChecklistPage = async (page = 1) => {
  ...
  try {
    ...
    const response = await fetch(`${route('employee.work.tasks.checklists.index', checklistModal.task.id)}?${params.toString()}`, {
      headers: {
        Accept: 'application/json',
        'X-Requested-With': 'XMLHttpRequest',
      },
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data.message || 'Unable to load checklist items.');
