    ...
    <div class="min-h-screen bg-[#f8fafc] p-4 lg:p-8 pb-32">
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
                    <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <h3 class="text-sm font-black uppercase tracking-widest text-slate-700">Checklist Execution</h3>
                    </div>
                    <div class="p-6 space-y-4">
                        <div v-for="item in analytics.checklists" :key="item.id" class="border border-slate-100 rounded-2xl overflow-hidden group">
                            <div class="p-4 flex items-center justify-between bg-white cursor-pointer" @click="item.showLogs = !item.showLogs">
                                <div class="flex items-center gap-3">
                                    <div :class="item.is_completed ? 'bg-emerald-500 shadow-emerald-200' : 'bg-slate-100'" class="w-6 h-6 rounded-lg flex items-center justify-center transition-all shadow-lg">
                                    ...
                                    <p :class="item.is_completed ? 'text-slate-400' : 'text-slate-700'" class="text-xs font-bold truncate">{{ item.content }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
