                ...
                <div class="p-6 overflow-y-auto">
                    ...
                    <form @submit.prevent="submitCheckIn" class="grid grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1">
                            ...
                            <input v-model="form.phone" type="tel" class="w-full rounded-lg border-slate-300 focus:ring-purple-500 focus:border-indigo-500" required placeholder="+91 98765 43210">
                        </div>
                        <div class="col-span-1">
                        ...
                        <div class="col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1">Meeting Host</label>
        ...
        <div v-if="showInviteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div class="bg-white rounded-[2rem] shadow-xl max-w-lg w-full overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div class="flex items-center gap-2">
                        <h3 class="font-black text-lg text-slate-800">Invite Visitor</h3>
                        <button @click="showInvitationInfo" class="text-slate-400 hover:text-indigo-600">
                ...
                <div v-if="!invitationResult" class="p-6">
                    <form @submit.prevent="submitInvite" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                           ...
                           <div>
                               ...
                               <input v-model="inviteForm.company" type="text" class="w-full mt-1 border-slate-300 rounded-lg">
                           </div>
                        </div>
                        <div>
                         ...
                         <!-- We need purposes list prop, assuming we can pass generic ID or fetch -->
                         <!-- Since we didn't pass 'purposes' in Index controller yet, we might fallback or use hardcoded ID or rely on backend default if nullable -->
                         <!-- For robust fix, we should update Controller to pass purposes. For now, assuming ID 1 exists (from migration seed) -->
        ...
        <div class="space-y-6 font-outfit">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 transition-all hover:shadow-2xl shadow-slate-200/50 group relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            <div class="text-sm font-black text-slate-400 uppercase tracking-widest">Active Violations</div>
                            <div class="text-2xl font-black text-slate-900">{{ props.stats.pulse?.active_violations || 0 }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 transition-all hover:shadow-2xl shadow-slate-200/50 group relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            <div class="text-sm font-black text-slate-400 uppercase tracking-widest">Awaiting Host</div>
                            <div class="text-2xl font-black text-slate-900">{{ props.stats.pulse?.unacknowledged || 0 }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 transition-all hover:shadow-2xl shadow-slate-200/50 group relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            <div class="text-sm font-black text-slate-400 uppercase tracking-widest">Security Ledger</div>
                            <div class="text-2xl font-black text-slate-900">{{ props.stats.pulse?.security_alerts || 0 }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 transition-all hover:shadow-2xl shadow-slate-200/50 group relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            <div class="text-sm font-black text-slate-400 uppercase tracking-widest">Traffic Pulse</div>
                            <div class="text-2xl font-black text-slate-900">{{ (props.stats.pulse?.traffic_delta || 0) > 0 ? '+' : '' }}{{ props.stats.pulse?.traffic_delta || 0 }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/40 backdrop-blur-xl p-6 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-white flex flex-wrap lg:flex-nowrap items-center justify-between gap-6 relative z-10">
                ...
                <div class="flex flex-wrap lg:flex-nowrap items-center gap-3 w-full lg:w-auto mt-4 lg:mt-0">
                    <a :href="route('visitors.export')" class="flex-1 lg:flex-none h-12 px-6 bg-white border border-slate-200 text-slate-600 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95">
                        ...
                        <div class="space-y-3">
                            <div v-for="visit in visitors_inside" :key="visit.id" class="flex justify-between items-start text-sm border-b border-slate-100 last:border-0 pb-2">
                                ...
                                <div class="text-right">
                                    <button @click="checkOut(visit.id)" class="text-xs text-red-600 hover:text-red-800 font-medium">
                                        Check Out
                                    </button>
                            ...
                            >
                                ...
                                <div class="p-5">
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="mt-3 flex items-center gap-2">
                                                ...
                                                <span class="text-xs text-slate-600 font-medium truncate">Host: {{ visit.visitor.host?.name }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-4 flex gap-2">
                                        ...
                                        <button v-if="visit.badge_collected_at" class="px-3 py-1.5 bg-green-50 text-green-600 rounded-lg border border-green-100" title="Badge Collected">
                        ...
                        <div class="divide-y divide-gray-100">
                            <div v-for="fam in dependents" :key="fam.id" class="p-4 flex justify-between items-center hover:bg-slate-50 transition-colors">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button @click="form.name = fam.name; form.phone = fam.phone; form.host_id = fam.employee?.user?.id; activeTab = 'console'" class="px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-700 font-medium hover:bg-slate-50 hover:text-indigo-600 transition-colors shadow-xl shadow-slate-200/50">
                                        Check In
                                    </button>
                                </div>
                            </div>
