    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600">With Accounts</p>
                        <p class="text-2xl font-bold text-purple-600">{{ withAccountCount }}</p>
                    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="contact in contacts" :key="contact.id" class="hover:bg-gray-50">
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <div class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                                            <span class="text-blue-600 font-semibold text-sm">{{ getInitials(contact.first_name, contact.last_name) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ contact.account?.name || 'No Account' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500">{{ contact.title || 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span :class="getStatusClass(contact.status)" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
                                    {{ contact.status }}
                                </span>
                            </td>
...
const activeCount = computed(() => {
    return props.contacts.filter(c => c.status === 'active').length;
});

const inactiveCount = computed(() => {
    return props.contacts.filter(c => c.status === 'inactive').length;
});

const withAccountCount = computed(() => {
    return props.contacts.filter(c => c.account_id !== null).length;
});
