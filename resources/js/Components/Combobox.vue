...
const filteredItems = computed(() =>
  query.value === ''
    ...
    : props.items.filter((item) => {
        const text = getDisplay(item).toLowerCase().replace(/\s+/g, '');
        const search = query.value.toLowerCase().replace(/\s+/g, '');
        // Also allow searching by ID if it's an employee
        const idMatch = item.id ? String(item.id).includes(search) : false;
  ...
  <div class="">
    <Combobox v-model="selectedItem" :multiple="multiple" v-slot="{ open }">
      <div class="relative mt-1">
        <div
          class="relative w-full cursor-default overflow-hidden rounded-lg bg-white text-left shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm border border-gray-300 flex flex-wrap gap-1 p-1 pr-10 items-center min-h-[38px]"
        >
          <!-- Tags for Multiple Selection -->
          <template v-if="multiple && Array.isArray(selectedItem)">
             <span v-for="item in selectedItem" :key="item[valueKey]" class="inline-flex items-center gap-1 rounded bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 whitespace-nowrap">
                {{ getDisplay(item) }}
