            ...
            <div class="flex-1 overflow-hidden bg-white relative">
                ...
                <div v-show="activeTab === 'general'" class="h-full overflow-y-auto p-6 scroll-smooth custom-scrollbar">
                    <form @submit.prevent="submit" id="taskFullForm" class="space-y-6">
                        ...
                        <!-- Global Validation Error (like locked schedules) -->
                        <div v-if="form.errors.message" class="p-4 bg-rose-50 border border-rose-200 rounded-xl flex items-start gap-3">
                            ...
                            <div>
                                <h4 class="text-sm font-black text-rose-900">Action Blocked</h4>
                                <p class="text-xs font-bold text-rose-700 mt-0.5">{{ form.errors.message }}</p>
                            </div>
                        </div>
        ...
        form.put(route('projects.tasks.update', { project: form.project_id, task: form.id }), {
            ...
            onError: (errors) => {
                if (errors.message) {
                    ...
                    toast.error('Unable to update. Please review the errors.');
                }
            }
        });
    } else {
        form.post(route('projects.tasks.store', form.project_id), {
            ...
            onError: (errors) => {
                if (errors.message) {
                    ...
                    toast.error('Unable to create. Please review the errors.');
                }
            }
        });
    }
