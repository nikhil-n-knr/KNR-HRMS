<template>
  <div class="space-y-6">
    ...
    >
        ...
        <template #cell-is_encashable="{ item }">
             <span class="text-xs font-semibold" :class="item.is_encashable ? 'text-emerald-600' : 'text-gray-450'">
                {{ item.is_encashable ? 'Yes' : 'No' }}
             </span>
        </template>

        <template #cell-requires_document="{ item }">
             <span class="text-xs font-semibold" :class="item.requires_document ? 'text-blue-600' : 'text-gray-450'">
                {{ item.requires_document ? 'Yes' : 'No' }}
             </span>
        </template>

        <template #cell-validity="{ item }">
             <span class="text-xs text-gray-600">
                <span v-if="item.applicable_from || item.applicable_to">
                    {{ item.applicable_from ? item.applicable_from.substring(0,10) : '...' }} to {{ item.applicable_to ? item.applicable_to.substring(0,10) : '...' }}
                </span>
                <span v-else class="text-gray-400 font-medium">No Date Limit</span>
             </span>
        </template>
        ...
        <template #rowActions="{ item }">
            <div class="flex justify-end gap-2">
                <button 
                    ...
                    class="p-1.5 bg-white rounded-lg border shadow-sm transition-colors"
                    :class="item.is_active ? 'text-amber-600 border-amber-100 hover:bg-amber-50' : 'text-emerald-600 border-emerald-100 hover:bg-emerald-50'"
                >
                    <svg v-if="item.is_active" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>
                    <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                </button>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <BaseInput 
                    ...
                    placeholder="e.g. 5" 
                    restrict="numbers"
                    :error="errors.carry_forward_limit"
                />
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1">
                    <label class="block text-sm font-medium text-gray-700">Applicable From</label>
                    <input type="date" v-model="form.applicable_from" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">
                    <span v-if="errors.applicable_from" class="text-xs text-red-500">{{ errors.applicable_from[0] }}</span>
                </div>
                <div class="space-y-1">
                    <label class="block text-sm font-medium text-gray-700">Applicable To</label>
                    <input type="date" v-model="form.applicable_to" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm">
                    <span v-if="errors.applicable_to" class="text-xs text-red-500">{{ errors.applicable_to[0] }}</span>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-6 pt-2">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" v-model="form.requires_document" class="rounded text-emerald-600 focus:ring-emerald-500">
                    <span class="text-sm font-medium text-gray-700">Document Required</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" v-model="form.is_encashable" class="rounded text-emerald-600 focus:ring-emerald-500">
                    <span class="text-sm font-medium text-gray-700">Is Encashable</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" v-model="form.is_active" class="rounded text-emerald-600 focus:ring-emerald-500">
                    <span class="text-sm font-medium text-gray-700">Active <span class="text-xs text-gray-400">(inactive types are hidden from employees)</span></span>
                </label>
            </div>
        </div>
        <template #footer>
...
const columns = {
    name: { label: 'Name' },
    code: { label: 'Code' },
    days_allowed_per_year: { label: 'Allowance' },
    carry_forward_limit: { label: 'Carry Fwd' },
    is_encashable: { label: 'Encashable' },
    requires_document: { label: 'Doc Req' },
    validity: { label: 'Active Period' },
    color: { label: 'Badge' },
    status: { label: 'Status' }
};
...
const openModal = (item = null) => {
    ...
    if (item) {
        ...
        form.value = { 
            ...item,
            applicable_from: item.applicable_from ? item.applicable_from.substring(0, 10) : '',
            applicable_to: item.applicable_to ? item.applicable_to.substring(0, 10) : ''
        };
    } else {
...
const toggleActive = async (item) => {
    try {
        const res = await axios.post(`/api/admin/leave-types/${item.id}/toggle-active`);
        // Update in local list
        const idx = types.value.findIndex(t => t.id === item.id);
        if (idx !== -1) types.value[idx].is_active = res.data.is_active;
        toast.success(res.data.is_active ? `${item.name} activated` : `${item.name} deactivated`);
    } catch (e) {
