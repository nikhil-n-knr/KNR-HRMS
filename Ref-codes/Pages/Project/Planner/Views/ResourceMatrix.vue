   ...
   <div class="h-full bg-white overflow-hidden flex flex-col relative">
     ...
     <Modal :show="showDetailsModal" @close="showDetailsModal = false" maxWidth="sm">
        <div class="p-4">
            ...
            <div v-if="getAvailabilityAlert(selectedRes?.id, selectedDateStr)" class="mb-4 p-3 rounded-xl border flex flex-col gap-1 shadow-sm transition-all" :class="getAvailabilityAlert(selectedRes?.id, selectedDateStr).classes">
                <div class="flex items-center justify-between">
                    <span class="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-white/40">
                        {{ getAvailabilityAlert(selectedRes?.id, selectedDateStr).type }}
                    </span>
                    <span class="text-xs font-medium opacity-90">
                        {{ getAvailabilityAlert(selectedRes?.id, selectedDateStr).status }}
                    </span>
                </div>
                <div class="text-sm font-bold mt-1">
                    {{ getAvailabilityAlert(selectedRes?.id, selectedDateStr).name }}
                </div>
            </div>
            ...
            <div class="space-y-2">
                <div v-for="task in cellTasks" :key="task.id" class="p-3 bg-gray-50 rounded-lg border border-gray-100 hover:border-indigo-300 transition-colors cursor-pointer group" @click="editAssignment(task)">
                    <div class="flex flex-col flex-1">
                        ...
                        <div v-if="task.project_name" class="text-[10px] font-black text-indigo-500 uppercase tracking-widest mt-0.5">
                            {{ task.project_code || 'PROJ' }} · {{ task.project_name }}
                        </div>
                    </div>
             ...
             <form @submit.prevent="submitAssignment">
                 <div class="space-y-5">
                     ...
                     <div>
                         ...
                         <div v-if="isEditing" class="mt-1 block w-full bg-gray-50 border border-gray-300 text-gray-500 rounded-md shadow-sm text-sm px-3 py-2 cursor-not-allowed">
                             {{ formatTaskDisplay(editingTask) }}
                         </div>
                         <TaskCombobox
                             v-else
                             v-model="form.task_id"
                             :items="pendingTasks"
                             :displayFormat="formatTaskDisplay"
                             label-key="text"
                             value-key="id"
                             placeholder="Search and choose a task..."
                             class="mt-1"
                         />
                         <p v-if="!isEditing && pendingTasks.length === 0" class="text-xs text-orange-500 mt-1">No pending tasks found.</p>
                     </div>

                     <!-- Task Timeline & Extensions Context -->
                     <div v-if="editingTask" class="bg-indigo-50/50 rounded-xl p-4 border border-indigo-100/50 space-y-3">
                         ...
                         <div class="grid grid-cols-2 gap-3 text-xs">
                             <div>
                                 <span class="text-gray-500 block">Baseline End Date</span>
                                 <span class="font-bold text-gray-800">{{ editingTask.baseline_due_date ? dayjs(editingTask.baseline_due_date).format('DD MMM YYYY') : 'Not Baselined' }}</span>
                             </div>
                             <div>
                                 <span class="text-gray-500 block">Current Task Due Date</span>
                                 <span class="font-bold text-gray-800 flex items-center gap-1">
                                     {{ editingTask.due_date ? dayjs(editingTask.due_date).format('DD MMM YYYY') : 'None' }}
                                     <span v-if="editingTask.baseline_due_date && editingTask.due_date !== editingTask.baseline_due_date" class="text-[10px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded font-black uppercase">Extended</span>
                                 </span>
                             </div>
                         </div>

                         <!-- Extensions List -->
                         <div v-if="editingTask.extensions && editingTask.extensions.length > 0" class="pt-2 border-t border-indigo-100/50 space-y-1.5">
                             <span class="text-[10px] font-black text-indigo-900 uppercase tracking-widest block">Approved Extensions / Drift</span>
                             <div class="space-y-1 max-h-24 overflow-y-auto pr-1">
                                 <div v-for="(ext, idx) in editingTask.extensions" :key="idx" class="flex justify-between items-start text-[11px] bg-white p-2 rounded border border-indigo-50">
                                     <div class="flex flex-col">
                                         <span class="font-bold text-gray-800">{{ ext.category_label || ext.category }}</span>
                                         <span class="text-gray-500 italic truncate max-w-[200px]" :title="ext.reason">{{ ext.reason }}</span>
                                     </div>
                                     <div class="text-right flex flex-col font-medium text-indigo-600">
                                         <span>+{{ ext.days_added }} days</span>
                                         <span class="text-[10px] text-gray-400 font-normal">+{{ ext.hours_added }} hrs</span>
                                     </div>
                                 </div>
                             </div>
                         </div>
                     ...
                     <div class="grid grid-cols-2 gap-4">
                         <div>
                             ...
                             <TextInput v-model="form.start_date" type="date" class="mt-1 block w-full disabled:opacity-60" required :disabled="isEditing" />
                         </div>
                         <div>
                             ...
                             <div class="relative mt-1">
                                 <TextInput 
                                     ...
                                     type="date" 
                                     class="block w-full disabled:opacity-60 pr-10" 
                                     required 
                                     :disabled="isEditing && !forceEndEditable" 
                                 />
                                 <!-- Sync Icon/Button Overlay if Extended -->
                                 <div v-if="isTaskExtended && !forceEndEditable" class="absolute inset-y-0 right-0 flex items-center pr-2">
                                     <button 
                                         type="button"
                                         @click="syncWithTaskExtension"
                                         class="p-1 text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 rounded transition-colors"
                                         title="Task has been extended. Click to sync allocation."
                                     >
                             ...
                             <!-- Sync Helper Text -->
                             <p v-if="isTaskExtended && !forceEndEditable" class="text-[10px] text-indigo-600 mt-1 font-medium flex items-center gap-1">
                                 <span>ℹ️ Task extended to {{ dayjs(editingTask.due_date).format('DD MMM') }}. Click the sync button to update allocation end date.</span>
                             </p>
                         </div>
                     </div>
                         ...
                         <div v-else class="pt-2 border-t border-gray-200">
                             ...
                             <div class="max-h-40 overflow-y-auto space-y-1 pr-1 border border-gray-200 rounded-md bg-white">
                                 <div v-for="day in dailyBreakdownDays" :key="day.dateStr" class="flex items-center justify-between px-2 py-1.5 border-b border-gray-100 last:border-0 hover:bg-gray-50">
                                     ...
                                     <div class="flex items-center gap-2">
                                         <!-- Exclude Checkbox -->
                                         <label class="inline-flex items-center text-xs text-red-600 font-medium cursor-pointer gap-1">
                                             <input 
                                                 type="checkbox" 
                                                 :checked="isDayExcluded(day.dateStr)" 
                                                 @change="toggleDayExclude(day.dateStr, $event.target.checked)"
                                                 class="rounded text-red-600 focus:ring-red-500 w-3.5 h-3.5"
                                             />
                                             <span>Off/Exclude</span>
                                         </label>
                                         
                                         <!-- Hours Input -->
                                         <TextInput 
                                             v-model="form.daily_allocations[day.dateStr]" 
                                             type="number" 
                                             min="0" 
                                             max="24" 
                                             step="0.5" 
                                             class="w-16 text-center h-8 text-xs px-2 py-1 disabled:opacity-50 disabled:bg-gray-100" 
                                             :disabled="isDayExcluded(day.dateStr)"
                                         />
                                     </div>
                                 </div>
                             </div>
...
import TaskCombobox from '@/Components/Combobox.vue';
...
const formatTaskDisplay = (task) => {
    if (!task) return '';
    const proj = task.project_code || '';
    return proj ? `[${proj}] ${task.text}` : task.text;
};
...
const forceEndEditable = ref(false);

const isTaskExtended = computed(() => {
    if (!isEditing.value || !editingTask.value || !form.end_date) return false;
    const taskEnd = dayjs(editingTask.value.due_date);
    const allocEnd = dayjs(form.end_date);
    return taskEnd.isAfter(allocEnd, 'day');
});

const syncWithTaskExtension = () => {
    if (editingTask.value && editingTask.value.due_date) {
        form.end_date = editingTask.value.due_date;
        forceEndEditable.value = true;
        toast.success(`Allocation end date synced to task end date: ${form.end_date}`);
    }
};

watch(showAssignmentModal, (val) => {
    if (!val) {
        forceEndEditable.value = false;
    }
});

const isDayExcluded = (dateStr) => {
    const val = form.daily_allocations[dateStr];
    return val === 0 || val === '0' || val === null || val === '';
};

const toggleDayExclude = (dateStr, checked) => {
    if (checked) {
        form.daily_allocations[dateStr] = 0;
    } else {
        form.daily_allocations[dateStr] = form.hours || 8;
    }
};
...
const dailyBreakdownDays = computed(() => {
    ...
    limitedDays.forEach(d => {
        if (form.daily_allocations[d.dateStr] === undefined) {
            const isSunday = dayjs(d.dateStr).day() === 0;
            form.daily_allocations[d.dateStr] = isSunday ? 0 : form.hours;
        }
    });
    return limitedDays;
});
        ...
        if (t.type !== 'project' && t.assignments) {
             ...
             userAssignments.forEach(assignment => {
                 ...
                 if (dateStr >= start && dateStr <= end) {
                    ...
                    const isLeave = (avail && avail.type === 'leave' && (avail.status === 'approved' || avail.status === 'Approved'));
...
const getCellClass = (userId, dateStr) => {
    ...
    const isApprovedLeave = (avail && avail.type === 'leave' && (avail.status === 'approved' || avail.status === 'Approved'));
    const isPendingLeave = (avail && avail.type === 'leave' && (avail.status === 'pending' || avail.status === 'Pending'));
    const isApprovedWfh = (avail && avail.type === 'wfh' && (avail.status === 'approved' || avail.status === 'Approved'));
    const isPendingWfh = (avail && avail.type === 'wfh' && (avail.status === 'pending' || avail.status === 'Pending'));
    
    const isBlocked = !!holiday || isApprovedLeave || nonWork;

    // 2. High Priority: Leave Conflicts (Load > 0 on a Leave Day)
    if (load > 0 && (isApprovedLeave || isPendingLeave)) {
        return 'bg-purple-200 border-2 border-red-500 text-purple-950 font-black line-through decoration-red-500 shadow-inner';
    }

    // 3. High Priority: Forced Work on Holiday / Weekend
    if (load > 0 && (holiday || nonWork)) {
        return 'bg-emerald-900 border-2 border-red-500 text-white font-bold shadow-inner';
    }

    // 4. WFH with active assignments (Working from Home)
    if (load > 0 && isApprovedWfh) {
        return 'bg-cyan-100 border-2 border-cyan-400 text-cyan-950 font-bold hover:bg-cyan-200';
    }
    if (load > 0 && isPendingWfh) {
        return 'bg-teal-50 border-2 border-dashed border-teal-300 text-teal-900 font-bold hover:bg-teal-100 pattern-diagonal-lines-sm';
    }

    // 5. Blocked Days / Specific Availabilities (If Load is 0)
    if (isApprovedLeave) {
        return 'bg-purple-100 text-purple-900 border border-purple-300 line-through decoration-purple-500 font-bold cursor-not-allowed';
    }
    if (isPendingLeave) {
        return 'bg-fuchsia-50 text-fuchsia-700 border border-dashed border-fuchsia-300 line-through decoration-fuchsia-400 font-medium cursor-not-allowed pattern-diagonal-lines-sm';
    }
    if (isApprovedWfh) {
        return 'bg-cyan-100 text-cyan-850 border border-cyan-300 font-bold cursor-pointer hover:bg-cyan-200';
    }
    if (isPendingWfh) {
        return 'bg-teal-50 text-teal-800 border border-dashed border-teal-300 font-medium cursor-pointer hover:bg-teal-100 pattern-diagonal-lines-sm';
    }
    
    if (holiday) {
         if (holiday.name && holiday.name.toLowerCase().includes('floating')) {
             return 'bg-yellow-100 text-yellow-800 pattern-diagonal-lines-sm cursor-not-allowed border border-yellow-200';
         }
    ...
    if (nonWork) {
         return 'bg-gray-100 text-gray-400 pattern-diagonal-lines-sm cursor-not-allowed border border-gray-200';
    }
...
const getCellTooltip = (userId, dateStr) => {
    ...
    if (holiday) {
        parts.push(`Holiday: ${holiday.name}`);
    }
    if (nonWork) {
        parts.push('Non-working day');
    }
    if (avail) {
        const typeLabel = avail.type === 'leave' ? 'Leave' : 'WFH';
        const statusLabel = avail.status === 'approved' ? 'Approved' : 'Pending Approval';
        parts.push(`${typeLabel} (${statusLabel}): ${avail.name.replace(/^(Leave|WFH):\s*/, '')}`);
    }
...
const getAvailabilityAlert = (userId, dateStr) => {
    ...
    if (avail) {
        if (avail.type === 'leave') {
            const isApproved = avail.status === 'approved' || avail.status === 'Approved';
            return {
                type: 'Leave',
                status: isApproved ? 'Approved' : 'Pending Approval',
                name: avail.name.replace(/^(Leave|WFH):\s*/, ''),
                classes: isApproved 
                    ? 'bg-purple-50 border-purple-200 text-purple-800' 
                    : 'bg-fuchsia-50 border-fuchsia-200 text-fuchsia-850 border-dashed'
            };
        } else if (avail.type === 'wfh') {
            const isApproved = avail.status === 'approved' || avail.status === 'Approved';
            return {
                type: 'WFH',
                status: isApproved ? 'Approved' : 'Pending Approval',
                ...
                classes: isApproved 
                    ...
                    : 'bg-teal-50 border-teal-200 text-teal-850 border-dashed'
            };
        }
    }
...
const handleCellClick = (res, day) => {
     ...
     const avail = getAvailability(res.id, day.dateStr);
     const holiday = isGlobalHoliday(day.dateStr);
     const nonWork = isNonWorkDay(day.dateStr);

     if (load > 0 || avail || holiday || nonWork) {
         // Open Details Modal
...
const submitAssignment = async () => {
    ...
    if (editingTask.value && editingTask.value.due_date && dayjs(form.end_date).isAfter(dayjs(editingTask.value.due_date))) {
        form.setError('date_range', `Allocation end date cannot exceed task due date (${editingTask.value.due_date})`);
        toast.error(`Allocation end date cannot exceed task due date (${editingTask.value.due_date})`);
        return;
    }
