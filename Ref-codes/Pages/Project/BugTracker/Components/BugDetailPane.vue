            ...
            <div class="flex-1 overflow-y-auto">
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <!-- Attachments -->
                        <div v-if="allAttachments.length">
                            <h3 class="text-sm font-bold text-gray-700 uppercase tracking-wider mb-2">Attachments</h3>
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                                <div v-for="file in allAttachments" :key="file.id" class="relative group border rounded-xl overflow-hidden shadow-sm bg-white hover:shadow-md transition-shadow">
                                    <!-- Image Preview -->
                                    <img v-if="isImage(file.mime)" :src="'/storage/' + file.path" class="h-32 w-full object-cover">
                                    <div v-else class="h-32 w-full flex flex-col items-center justify-center bg-gray-50 border-b border-gray-100">
                                        <div class="h-10 w-10 bg-slate-200 rounded-lg flex items-center justify-center text-slate-500 font-black uppercase text-[10px]">
                                            {{ file.name.split('.').pop() }}
                                        </div>
                                        <span class="mt-2 text-[10px] font-bold text-gray-500 truncate px-2 w-full text-center">{{ file.name }}</span>
                                    </div>
                                    
                                    <a :href="'/storage/' + file.path" target="_blank" 
                                       class="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-white text-xs font-black uppercase tracking-widest">
                                        View Full
                                    </a>
                                </div>
                            </div>
...
const allAttachments = computed(() => {
    ...
    // 1. Legacy JSON attachments
    if (Array.isArray(bug.value?.attachments)) {
        bug.value.attachments.forEach(a => {
            list.push({
                id: `json-${a.path}`,
                name: a.name,
                path: a.path,
    ...
    // 2. New Polymorphic media
    if (Array.isArray(bug.value?.media)) {
        bug.value.media.forEach(m => {
            list.push({
                id: `media-${m.id}`,
