    ...
    <div v-if="show" class="fixed inset-0 overflow-hidden z-[110]">
        ...
        <div class="fixed inset-y-0 right-0 max-w-sm w-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right-full duration-300">
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-8">
                ...
                <!-- Date Range Filter -->
                <div>
                    <InputLabel value="Date Range" class="text-sm font-black uppercase tracking-widest text-slate-500 mb-3" />
                    <div class="flex items-center gap-2">
                        <input type="date" v-model="localFilters.date_start" class="w-full bg-slate-50 border-slate-200 text-slate-600 text-sm font-bold rounded-xl focus:ring-indigo-500 px-3 py-2" title="From Date" />
                        <span class="text-slate-300 font-black">-</span>
                        <input type="date" v-model="localFilters.date_end" class="w-full bg-slate-50 border-slate-200 text-slate-600 text-sm font-bold rounded-xl focus:ring-indigo-500 px-3 py-2" title="To Date" />
                    </div>
                </div>

                <!-- Projects Filter (Multi-select) -->
                <div>
                    <InputLabel value="Projects" class="text-sm font-black uppercase tracking-widest text-slate-500 mb-3" />
                    <div class="border border-slate-200 rounded-xl p-3 bg-slate-50/50 hover:bg-white hover:border-indigo-200 transition-all focus-within:bg-white focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500">
                        <select multiple v-model="localFilters.project_ids" class="block w-full border-0 bg-transparent text-sm focus:ring-0 p-0 h-32">
                            <option v-for="(name, id) in lookup.projects" :key="'proj_'+id" :value="String(id)" class="py-1 px-2 mb-1 rounded-md text-slate-700 font-medium hover:bg-indigo-50 checked:bg-indigo-100">{{ name }}</option>
                        </select>
...
const syncLocalFilters = () => {
    ...
    localFilters.value = {
        ...
        project_ids: Array.isArray(props.filters.project_ids) ? [...props.filters.project_ids] : (props.filters.project_ids ? [props.filters.project_ids] : (props.filters.project_id ? [props.filters.project_id] : [])),
