    ...
    <div :class="{'max-w-[1200px] mx-auto': !embedded}" class="space-y-6 pb-12">
        ...
        <div class="sticky top-0 z-40 bg-white/90 backdrop-blur-md rounded-2xl border border-slate-200 p-2 shadow-sm flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <nav class="flex bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-inner overflow-x-auto no-scrollbar">
                <button 
                    v-for="t in ['single', 'bulk', 'bulk_mark']"
                    :key="t"
                    @click="activeTab = t"
                    class="px-5 py-2 text-sm font-black uppercase tracking-widest rounded-lg transition-all whitespace-nowrap"
                    :class="activeTab === t ? 'bg-white text-emerald-600 shadow-sm border border-slate-200/50' : 'text-slate-400 hover:text-slate-600'"
                >
                    <i :class="getTabIcon(t)" class="mr-2 text-sm"></i>
                    {{ t.replace('_', ' ') }}
                </button>
                <Link 
                    :href="route('admin.attendance.bulk-manage')"
                    class="px-5 py-2 text-sm font-black uppercase tracking-widest rounded-lg transition-all whitespace-nowrap text-slate-450 hover:text-emerald-600 flex items-center hover:bg-white/50"
                >
                    ...
                    <div class="bg-emerald-50/80 p-5 rounded-2xl border border-emerald-100 flex flex-col sm:flex-row items-center gap-5 shadow-sm">
                        ...
                        <div class="flex-1 flex flex-wrap items-center gap-4">
                             ...
                             <div class="flex flex-wrap gap-2 overflow-x-auto pb-2 sm:pb-0 no-scrollbar">
                                <button 
                                    v-for="status in ['Present', 'Late', 'Work From Home', 'Absent', 'Half Day', 'On Leave']" 
                                    :key="status"
...
const getInitialTab = (tabStr) => {
    if (!tabStr) return 'single';
    if (tabStr === 'manual_entry_single') return 'single';
    if (tabStr === 'manual_entry_bulk') return 'bulk';
    if (tabStr === 'manual_entry_bulk_mark') return 'bulk_mark';
    if (tabStr === 'manual_entry') return 'single';
    return tabStr;
};
const activeTab = ref(getInitialTab(props.tab));

watch(() => props.tab, (newVal) => {
    activeTab.value = getInitialTab(newVal);
});
