                            ...
                            <div class="flex-1 flex overflow-x-auto hide-scrollbar">
                                <div v-for="day in days" :key="day" class="min-w-[140px] w-full border-r border-gray-50 p-2.5 flex items-center justify-center">
                                    ...
                                    >
                                        ...
                                        <span class="text-sm font-black uppercase tracking-widest truncate w-full leading-none">{{ getShiftLabel(emp, day) }}</span>
                                        <div v-if="!getShiftForDay(emp, day).isDefault && !isOffDayForShift(getShiftForDay(emp, day), day)" class="flex items-center gap-1.5 mt-3 opacity-60 group-hover/shift:opacity-100 transition-opacity">
                        ...
                        <div class="flex overflow-x-auto gap-3 pb-2 hide-scrollbar">
                            <div v-for="day in days" :key="day" class="min-w-[100px] flex flex-col items-center group/mbshift">
                                ...
                                <div 
                                    ...
                                    :class="getShiftStyles(getShiftForDay(emp, day), day)"
                                >
                                    <span class="text-sm font-black uppercase truncate w-full leading-none">{{ getShiftLabel(emp, day) }}</span>
                                    <span v-if="!getShiftForDay(emp, day).isDefault && !isOffDayForShift(getShiftForDay(emp, day), day)" class="text-sm font-black mt-2 tabular-nums opacity-70">
                                        {{ getShiftForDay(emp, day).start_time.slice(0, 5) }}
                                    </span>
                                </div>
                            </div>
...
const getShiftForDay = (employee, date) => {
    ...
    const defaultShift = shifts.value.find(s => s.is_default) || shifts.value[0];
    return defaultShift ? { ...defaultShift, isDefault: true } : { name: '--', color: 'gray', isDefault: true };
};

const isOffDayForShift = (shift, date) => {
    if (!shift) return false;
    
    // Holidays are global off days
    const dateStr = date.toISOString().slice(0, 10);
    const isHoliday = currentHolidays.value.some(h => h.date === dateStr);
    if (isHoliday) return true;

    // Check work days array
    if (!shift.work_days) {
        return date.getDay() === 0; // Default Sunday off
    }

    const workDays = Array.isArray(shift.work_days)
        ? shift.work_days
        : JSON.parse(shift.work_days || '[]');
        
    const dayName = date.toLocaleDateString('en-US', { weekday: 'short' }); // Mon, Tue...
    const isWeeklyOff = !workDays.some(d => d.toLowerCase() === dayName.toLowerCase());
    
    if (isWeeklyOff) return true;

    // Check Nth week off rules
    const rules = Array.isArray(shift.week_off_rules)
    ...
    if (rules && rules.length > 0) {
        ...
        const isLastOccurrence = nextWeekDate.getMonth() !== date.getMonth();

        for (const rule of rules) {
            const ruleWeekday = rule.weekday;
            if (ruleWeekday && ruleWeekday.substring(0, 3).toLowerCase() === dayName.toLowerCase()) {
                const weeks = rule.weeks || [];
                if (weeks.includes(weekOfMonth) || (isLastOccurrence && weeks.includes('last'))) {
                    return true;
                }
            }
        }
...
const getShiftLabel = (employee, day) => {
    const d = day.toISOString().slice(0, 10);
    const shift = getShiftForDay(employee, day);
    const hasAssignment = !!roster.value[employee.id]?.[d];

    // 1. Check if it's a holiday
    const holiday = currentHolidays.value.find(h => h.date === d);
    if (holiday) {
        return holiday.name || 'HOLIDAY';
    }

    // 2. Check if it's a week off
    if (isOffDayForShift(shift, day)) {
        return 'WEEK OFF';
    }
...
const getShiftStyles = (shift, day) => {
    const isOff = isOffDayForShift(shift, day);
    
    if (isOff) {
        return 'bg-rose-50/70 text-rose-600 border-rose-200 shadow-sm opacity-90 font-bold';
    }
...
const isOffDay = (date) => {
    ...
    return isOffDayForShift(defaultShift, date);
};
