                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        <tr v-for="member in families.data" :key="member.id" class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-bold text-gray-900">{{ member.employee?.first_name || 'Deleted' }} {{ member.employee?.last_name || 'Employee' }}</div>
                                <div class="text-xs text-gray-500">{{ member.employee?.employee_code || 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
            ...
            <div class="md:hidden divide-y divide-gray-100">
                <div v-for="member in families.data" :key="member.id" class="p-4 space-y-4">
                    <div class="flex items-center justify-between">
                         <div>
                            <div class="text-sm font-bold text-gray-900">{{ member.employee?.first_name || 'Deleted' }} {{ member.employee?.last_name || 'Employee' }}</div>
                            <div class="text-sm text-gray-500 font-bold uppercase tracking-widest">{{ member.employee?.employee_code || 'N/A' }}</div>
                        </div>
