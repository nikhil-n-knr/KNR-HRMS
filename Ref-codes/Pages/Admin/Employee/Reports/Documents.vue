                        ...
                        <tr v-for="doc in documents.data" :key="doc.id" class="hover:bg-gray-50/50 transition group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-sm font-bold text-gray-900">{{ doc.employee?.first_name || 'Deleted' }} {{ doc.employee?.last_name || 'Employee' }}</div>
                                        <div class="text-xs text-gray-500">{{ doc.employee?.employee_code || 'N/A' }} | {{ doc.employee?.department?.name || 'No Dept' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div v-for="doc in documents.data" :key="doc.id" class="p-4 space-y-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-sm font-bold text-gray-900">{{ doc.employee?.first_name || 'Deleted' }} {{ doc.employee?.last_name || 'Employee' }}</div>
                                <div class="text-sm text-gray-500 font-bold uppercase tracking-widest">{{ doc.employee?.employee_code || 'N/A' }}</div>
                            </div>
                        </div>
