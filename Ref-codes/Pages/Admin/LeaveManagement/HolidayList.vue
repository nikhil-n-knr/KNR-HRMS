...
const formatDate = (date, endDate = null) => {
    if (!date) return '';
    const startStr = new Date(date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' });
    if (!endDate || endDate === date) {
        return startStr;
    }
    const endStr = new Date(endDate).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' });
    ...
    <div :class="{'max-w-[1600px] mx-auto': !embedded}" class="space-y-12 animate-fade-in relative z-10 pb-24">
        ...
        >
            ...
            <template #cell-date="{ item }">
                <span class="font-medium text-gray-900" :class="{'text-gray-400': item.is_hidden_for_year}">{{ formatDate(item.date, item.end_date) }}</span>
            </template>
        ...
        >
            <form @submit.prevent="submit" class="space-y-10">
                ...
                <div class="grid grid-cols-3 gap-6">
                    <div class="space-y-4">
                        <label class="text-sm font-black text-slate-400 uppercase tracking-widest ml-2">Start Date</label>
                    ...
                    <div class="space-y-4">
                        <label class="text-sm font-black text-slate-400 uppercase tracking-widest ml-2">End Date (Optional)</label>
                        <input type="date" v-model="form.end_date" class="w-full bg-slate-50 border-transparent rounded-[24px] py-5 px-8 text-sm font-black text-slate-700 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all uppercase tracking-widest">
                        <p v-if="errors.end_date" class="text-sm font-black text-rose-500 uppercase tracking-widest mt-2 px-1">{{ errors.end_date[0] }}</p>
                    </div>
