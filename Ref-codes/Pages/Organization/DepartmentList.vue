<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-violet-800 to-indigo-700">
          Departments
        </h1>
        <p class="text-violet-600/80 text-sm mt-1">Manage organization departments</p>
      </div>
      <button 
        v-if="authStore.can('org.departments.create')"
        @click="openModal()"
        class="px-4 py-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl text-sm font-medium shadow-lg shadow-violet-500/30 hover:shadow-violet-500/40 hover:-translate-y-0.5 transition-all flex items-center gap-2"
      >
    ...
    <Modal :show="showModal" :title="isEditing ? 'Edit Department' : 'Create Department'" @close="closeModal">
        ...
        <template #footer>
            ...
            <button 
                ...
                class="px-4 py-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-lg text-sm font-medium shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
