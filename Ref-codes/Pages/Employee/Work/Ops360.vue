      ...
      <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <a v-if="false" :href="opsLaunchers.devops_global_url" class="h-10 px-4 rounded-xl bg-emerald-600 text-white text-xs font-black uppercase tracking-wider inline-flex items-center justify-center">Squad_Ops</a>
          </div>
        </div>
      </section>
      ...
      <section v-else class="grid grid-cols-1 xl:grid-cols-[1fr_1fr] gap-4">
        <article class="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-5 shadow-sm space-y-3">
          ...
          <div class="pt-2 space-y-1 max-h-24 overflow-auto pr-1">
            <a v-if="false" v-for="project in opsLaunchers.project_devops || []" :key="`devops-project-${project.id}`" :href="project.url" class="block text-xs font-bold text-emerald-700 hover:text-emerald-800 truncate">{{ project.name }} DevOps</a>
            <p v-if="false" class="text-xs font-semibold text-slate-400">No project DevOps links available.</p>
          </div>
        </article>
