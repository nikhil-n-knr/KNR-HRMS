...
class AttendanceController extends Controller
{
    ...
    /**
     * Standalone Independent View for Live Attendance Monitoring
     */
    public function liveMonitorStandalone(Request $request)
    {
        $today = now()->format('Y-m-d');
        
        // Fetch current day logs
        $logs = AttendanceLog::with(['employee.department', 'sessions'])
            ->where('date', $today)
            ->latest()
            ->paginate(20);
        ...
        $stats = [
            ...
            'present' => AttendanceLog::where('date', $today)->whereIn('status', ['Present', 'Late'])->count(),
            'late' => AttendanceLog::where('date', $today)->where('status', 'Late')->count(),
            'absent' => 0, // Calculated below
        ];
        $stats['absent'] = $stats['total_employees'] - $stats['present'];

        return \Inertia\Inertia::render('Admin/Attendance/LiveMonitorStandalone', [
            'logs' => $logs,
            'stats' => $stats,
            'today' => now()->format('l, jS F Y')
        ]);
    }

    /**
     * Hidden bulk management page for marking attendance.
     */
    public function bulkManagePage(Request $request)
    {
        $tenantId = auth()->user()->tenant_id;
        ...
        // 2. Determine selected parameters
        $selectedEmployeeId = (int) $request->input('employee_id', auth()->user()->employee?->id ?? $employees->first()['id'] ?? 0);
        ...
        if ($selectedEmployeeId > 0) {
            ...
            if ($employee) {
                ...
                while ($tempDate->lte($endDate)) {
                    ...
                    $isWeekOff = $resolver->isWeekOff($tempDate, $employee);
                    $isHoliday = $resolver->isHoliday($tempDate, $employee);
                    
                    $defaultStatus = 'present';
                    if ($isHoliday) {
                        $defaultStatus = 'holiday';
                    } elseif ($isWeekOff) {
                        $defaultStatus = 'week_off';
                    }

                    $log = $logs->get($dateStr);
                    $session = $log ? $log->sessions->first() : null;

                    $days[] = [
                        'date' => $dateStr,
                        'day_name' => $tempDate->format('l'),
                        'is_weekend' => $isWeekOff || $isHoliday,
                        'status' => $log ? $log->status : $defaultStatus,
                        'in_time' => $session && $session->in_time ? $session->in_time->format('H:i') : '09:00',
                        'out_time' => $session && $session->out_time ? $session->out_time->format('H:i') : '18:00',
                        'is_new' => !$log,
                    ];
        ...
        // Return base data including tabs info for MainLayout
        $data = $this->getHubBaseData('bulk_manage', $tenantId);
        ...
        return \Inertia\Inertia::render('Admin/Attendance/BulkManage', $data);
    }

    /**
     * Save the bulk management records.
     */
    public function saveBulkManage(Request $request)
    {
        $request->validate([
            'employee_id' => 'required|exists:employees,id',
            'days' => 'required|array',
            'days.*.date' => 'required|date',
            'days.*.status' => 'required|string',
            'days.*.in_time' => 'nullable|string',
            'days.*.out_time' => 'nullable|string',
        ]);
        ...
        foreach ($request->days as $dayData) {
            ...
            $log = AttendanceLog::updateOrCreate(
                ...
                [
                    'status' => $status,
                    'shift_id' => 1, // Default General Shift
                ]
            );

            // Clear existing sessions for this date to avoid duplicates
            $log->sessions()->delete();

            // If status is working, insert session
            $isWorkingStatus = in_array(strtolower($status), ['present', 'late', 'half_day', 'halfday', 'work_from_home', 'wfh', 'wfh approved', 'wfh_approved']);
            if ($isWorkingStatus && !empty($dayData['in_time']) && !empty($dayData['out_time'])) {
                $in = Carbon::parse($date . ' ' . $dayData['in_time']);
                $out = Carbon::parse($date . ' ' . $dayData['out_time']);
                ...
                $log->update(['total_work_minutes' => 0]);
            }

            $savedCount++;
        }

        return back()->with('success', "Saved attendance for $savedCount days.");
    }
}
