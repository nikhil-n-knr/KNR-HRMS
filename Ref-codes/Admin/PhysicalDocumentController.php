...
class PhysicalDocumentController extends Controller
{
    ...
    public function updateRecord(Request $request, PhysicalRecord $record)
    {
    ...
    public function markFound(PhysicalRecord $record)
    {
        try {
            $this->service->markFound($record->id);
        } catch (\Exception $e) {
            return back()->with('error', $e->getMessage());
        }

        return back()->with('success', 'Document Marked Found');
    }
    ...
    public function destroyLocation(\App\Models\PhysicalDocumentLocation $location)
    {
         DB::transaction(function () use ($location) {
             $this->deleteLocationTree($location);
         });

         return back()->with('success', 'Location Deleted');
    }

    private function deleteLocationTree(\App\Models\PhysicalDocumentLocation $location): void
    {
        $location->load('children');

        foreach ($location->children as $child) {
            $this->deleteLocationTree($child);
        }
