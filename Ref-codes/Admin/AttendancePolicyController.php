...
class AttendancePolicyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $tab = $request->input('tab') ?? $request->route('tab') ?? 'policies';
        
        $data = $this->getHubBaseData($tab, $tenantId);
        $data['standalone'] = str_ends_with($request->route()->getName(), '.standalone');
        ...
        // 1.3 Daily Logs — All-Employee Attendance Matrix
        if ($tab === 'daily_log') {
            $monitoring = app(\App\Http\Controllers\Admin\MonitoringController::class);
            $matrixData = $monitoring->getAttendanceMatrixData($request, $tenantId);
            $data['matrix_employees'] = $matrixData['matrix_employees'];
            $data['matrix_stats']     = $matrixData['matrix_stats'];
        } else {
            $data['matrix_employees'] = \Inertia\Inertia::lazy(fn() => null);
            $data['matrix_stats']     = \Inertia\Inertia::lazy(fn() => null);
        }
