...
class WfhController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $rawHolidays = \App\Models\Holiday::where(function($q) use ($start, $end) {
            $q->where('date', '<=', $end->toDateString())
              ->where('end_date', '>=', $start->toDateString());
        })->where('type', 'Fixed')->get();

        $holidays = [];
        foreach ($rawHolidays as $h) {
            $curr = \Carbon\Carbon::parse($h->date);
            $hEnd = \Carbon\Carbon::parse($h->end_date ?? $h->date);
            while ($curr <= $hEnd) {
                $holidays[] = $curr->toDateString();
                $curr->addDay();
            }
        }
        ...
        if ($createdCount === 0) {
            ...
            return back()->with('error', 'No new requests created. Days might be Sundays, Holidays, or already requested.');
        }
