...
class LeaveRequestController extends Controller
{
        ...
        return \Illuminate\Support\Facades\DB::transaction(function() use ($leaveRequest, $validated, $request, $user) {
             ...
             if ($newDays <= 0) {
                  throw new \Illuminate\Validation\ValidationException(\Illuminate\Validation\ValidationException::withMessages([
                      'custom' => ["The selected range does not contain any working days."]
                  ]));
             }
             ...
             if ($newDays > $available) {
                  throw new \Illuminate\Validation\ValidationException(\Illuminate\Validation\ValidationException::withMessages([
                      'custom' => ["Insufficient balance. Requested: {$newDays}, Available: {$available}"]
                  ]));
             }
    ...
    public function destroy(LeaveRequest $leaveRequest)
    {
        ...
        return \Illuminate\Support\Facades\DB::transaction(function() use ($leaveRequest) {
            ...
            $this->logger->log('leave_management', 'cancel', "Leave request cancelled: {$leaveRequest->uuid}");
    ...
    public function action(Request $request, LeaveRequest $leaveRequest)
    {
        ...
        if ($leaveRequest->employee->reporting_to !== $user->id) {
             // For now, simple check. Super Admin bypass?
