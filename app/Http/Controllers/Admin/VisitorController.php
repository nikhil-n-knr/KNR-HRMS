...
class VisitorController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('category') && $request->category !== 'All Categories') {
            // Mapping categories to purpose workflows or keys if needed
            $query->whereHas('visitor', function($q) use ($request) {
                // This is a simplification, actual mapping depends on how categories are defined
        ...
        return Inertia::render('Admin/Visitors/Index', [
            'filters' => $request->all(['dateRange', 'status', 'category']),
            'stats' => [
                'currently_inside' => VisitorPass::whereNotNull('check_in_at')->whereNull('check_out_at')->count(),
                'expected_today' => VisitorPass::whereDate('visit_date', today())->whereNull('check_in_at')->count(),
            ...
            'events' => Event::with('organizer')->withCount('passes')->latest()->get(),
    ...
    {
        $passes = VisitorPass::with('visitor.host')->latest()->get();
        $csvHeader = ['ID', 'Visitor Name', 'Company', 'Host', 'Purpose', 'In', 'Out', 'Status'];
        ...
        $callback = function() use ($passes, $csvHeader) {
            ...
            foreach ($passes as $pass) {
                fputcsv($file, [
                    $pass->pass_code,
                    $pass->visitor->name,
                    $pass->visitor->company,
                    $pass->visitor->host->name,
